...
class HeartbeatConfig(BaseModel):
    ...
    # max_turns=0 is valid for in-process handlers (handler field set); otherwise >= 1
    ...
    # decision_prompt may be empty for in-process handlers (handler field set)
    decision_prompt: str = ""
    # in-process handler: "module_name.function_name" — bypasses Claude CLI subprocess
    handler: Optional[str] = None
    source_plugin: Optional[str] = None  # AC4: set to plugin slug for plugin-contributed heartbeats

    @model_validator(mode="after")
    def handler_vs_claude_constraints(self) -> "HeartbeatConfig":
        """Enforce different constraints for handler-based vs Claude-based heartbeats."""
        if self.handler:
            # In-process handlers: max_turns must be 0; decision_prompt not used
            if self.max_turns != 0:
                raise ValueError(
                    f"Heartbeat '{self.id}': handler-based heartbeats must have max_turns=0 "
                    f"(got {self.max_turns})"
                )
        else:
            # Claude CLI heartbeats: max_turns >= 1; decision_prompt required
            if self.max_turns < 1:
                raise ValueError(
                    f"Heartbeat '{self.id}': Claude-based heartbeats must have max_turns >= 1 "
                    f"(got {self.max_turns}). Use handler field for in-process workers."
                )
            if len(self.decision_prompt) < 20:
                raise ValueError(
                    f"Heartbeat '{self.id}': decision_prompt must have at least 20 characters "
                    f"(got {len(self.decision_prompt)}). "
                    "This is the question the agent answers in step 4 of the protocol."
                )
        return self
