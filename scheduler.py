...
def main():
    ...
    # Track which monthly routines already ran this month — key: "YYYY-MM-script"
    monthly_ran_keys: set[str] = set()
    ...
    while True:
        ...
        for r in _monthly_routines:
            if not r.get("enabled", True):
                continue
            # Respect per-routine day_of_month (default 1) and time (default "08:00")
            target_day = int(r.get("day_of_month", 1))
            target_time_str = r.get("time", "08:00")
            try:
                target_hour = int(target_time_str.split(":")[0])
            except (ValueError, AttributeError):
                target_hour = 8
            script_key = r.get("script", r.get("name", ""))
            run_key = f"{now.year}-{now.month:02d}-{script_key}"
            if now.day == target_day and now.hour == target_hour and run_key not in monthly_ran_keys:
                run_adw(r.get("name", r.get("script", "")), f"custom/{r['script']}", r.get("args", ""))
                monthly_ran_keys.add(run_key)

        # Purge keys from previous months to avoid unbounded growth
        current_prefix = f"{now.year}-{now.month:02d}-"
        monthly_ran_keys = {k for k in monthly_ran_keys if k.startswith(current_prefix)}
